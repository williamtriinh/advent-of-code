...
func parseInput(input string, byteCount int) []imath.Vec2 {
	lines := aoc.SplitAndTrim(input, "\n")

	bytePositions := make([]imath.Vec2, byteCount)
...
func solution(gridSize int, byteCount int, input string) int {
	bytePositions := parseInput(input, byteCount)
...
func TestSolution(t *testing.T) {
	...
	t.Log(solution(71, 1024, string(input)))
}
